 ...
 *
 * Brand tokens come from the shared `@feastpot/ui/brand` module (single
 * source of truth) so component authors can write `bg-brand`,
 * `text-teal-dark`, `border-vendor` without going through the shadcn HSL
 * CSS-variable layer (which is reserved for the neutral/semantic tokens
 * defined in `globals.css` and `@feastpot/ui/theme.css`).
 *
  ...
  theme: {
    ...
    extend: {
      colors: {
        ...
        // 2026-05-16 PWA redesign - wireframe palette. Brand green is the
        // primary CTA. All hexes now come from @feastpot/ui/brand so the
        // next palette change cascades to every app from one file.
        brand: { ...brandColors.brand },
        scotch: brandColors.scotch, // wireframe red - offers, discounts, urgent
        plantain: brandColors.plantain, // wireframe gold - rewards, FeastPass, highlights
        yam: brandColors.yam, // FSA, halal, verified - collapses to brand green
        pot: brandColors.pot, // neutral decorative
        ...
        vendor: brandColors.vendor.DEFAULT,
        ...
        surface: brandColors.surface.web,
