...
/**
 * Lightweight typed fetch wrapper for the Feastpot API.
 *
 * - All routes are versioned under `/v1` (NestJS URI versioning).
 * - 4xx/5xx responses become a thrown `ApiError` carrying the parsed body so
 *   callers (and TanStack Query) can pattern-match on `.code`.
 * - `accessToken` is optional: public endpoints (vendor search, vendor by
 *   slug, reviews list) work without it; private ones (e.g. /users/sync) get
 *   a `Bearer` header attached.
 */
...
export interface ApiRequestOptions {
  ...
  /**
   * Query params: arrays are joined with `,` (matches the API's
   * `SearchVendorsDto` toArray transformer); `undefined`/`null`/`''` are
   * dropped so callers can pass typed objects directly without pre-cleaning.
   */
  query?: QueryParams;
  body?: unknown;
  accessToken?: string;
...
function buildQuery(query: QueryParams | undefined): string {
  ...
  for (const [k, v] of Object.entries(query)) {
    ...
    if (Array.isArray(v)) {
      // Backend SearchVendorsDto accepts comma-separated arrays.
