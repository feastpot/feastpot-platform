  ...
  return (
    <section className="space-y-3 px-4 py-6">
      <header className="space-y-1">
        <h2 className="text-[17px] font-bold text-dark">Loved by communities across London</h2>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-mid">{t.area}</p>
              </div>
            </div>
            ...
            >
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  ...
                  className={cn(
                    'h-3.5 w-3.5',
                    i < t.rating ? 'fill-amber-400 text-amber-400' : 'text-mid/30',
                  )}
                  aria-hidden
                />
              ))}
            ...
            <span className="inline-flex w-fit items-center rounded-full bg-surface px-2.5 py-1 text-[11px] font-medium text-dark">
              {t.dish}
            </span>

            <p className="text-sm italic leading-relaxed text-mid">&ldquo;{t.quote}&rdquo;</p>
          </li>
        ))}
      </ul>
    </section>
