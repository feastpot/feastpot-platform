...
export default function EventEnquiryDetailPage() {
  ...
  if (error || !enquiry)
    return (
      <PageShell>
        <p className="py-8 text-center text-sm font-medium text-scotch">Enquiry not found.</p>
      </PageShell>
    );
  ...
  return (
    <PageShell>
      ...
      <header className="py-4">
        <h1 className="font-display text-2xl font-black capitalize tracking-tight text-charcoal">
          {enquiry.eventType}
        </h1>
        <dl className="mt-2 grid grid-cols-2 gap-1 text-sm font-medium text-charcoal-mid">
          <div>
            ...
            <dd className="inline text-charcoal">
              {new Date(enquiry.eventDate).toLocaleString('en-GB')}
            </dd>
          </div>
          <div>
            <dt className="inline">Guests: </dt>
            <dd className="inline text-charcoal">{enquiry.guestCount}</dd>
          </div>
          <div>
            <dt className="inline">Postcode: </dt>
            <dd className="inline text-charcoal">{enquiry.postcode}</dd>
          </div>
          <div>
            <dt className="inline">Cuisines: </dt>
            <dd className="inline text-charcoal">{enquiry.cuisines.join(', ') || '-'}</dd>
          </div>
        </dl>
        <p className="mt-3 text-xs font-medium text-charcoal-mid">
          Matched vendors:{' '}
          <strong className="font-bold text-charcoal">{enquiry.matchedVendorIds.length}</strong> ·
          Quotes received:{' '}
          <strong className="font-bold text-charcoal">{enquiry.quotes?.length ?? 0}</strong>
        </p>
      </header>
      ...
      {enquiry.status === 'confirmed' && (
        <div className="rounded-xl border border-brand/30 bg-brand/10 p-3 text-sm font-medium text-brand-dark">
          ...
          <Link href={`/events/${enquiry.id}/confirmed`} className="font-bold underline">
            View booking →
          </Link>
        </div>
      )}
      ...
      <section className="mt-4 space-y-3">
        <h2 className="font-display text-base font-black tracking-tight text-charcoal">
          Quotes received
        </h2>
          ...
          .map((q) => (
            <Elements key={q.id} stripe={stripePromise}>
              <QuoteCard
                ...
                canSelect={enquiry.status !== 'confirmed' && STRIPE_CONFIGURED}
              />
            </Elements>
          ))}
      </section>
  ...
  return (
    <article className="rounded-2xl border border-cream-deep bg-white p-4 shadow-sm">
      <header className="flex items-baseline justify-between">
        <h3 className="font-display text-base font-black text-charcoal">
          {quote.vendor?.businessName ?? 'Vendor'}
        </h3>
        <span className="text-xs font-bold text-charcoal-mid">
          <span className="text-plantain">★</span> {quote.vendor?.rating?.toFixed(1) ?? '-'}
        </span>
      </header>
      {quote.proposedMenu && (
        <p className="mt-2 whitespace-pre-line text-sm font-medium text-charcoal">
          {quote.proposedMenu}
        </p>
      )}
      <dl className="mt-3 grid grid-cols-2 gap-1 text-sm font-medium text-charcoal">
        <div>
          <dt className="inline text-charcoal-mid">Per head: </dt>
          <dd className="inline">
            {formatPounds(quote.perHeadPence)} × {guestCount}
          </dd>
        </div>
        <div>
          <dt className="inline text-charcoal-mid">Subtotal: </dt>
          <dd className="inline">{formatPounds(subtotal)}</dd>
        </div>
        <div>
          <dt className="inline text-charcoal-mid">Delivery: </dt>
          <dd className="inline">{formatPounds(quote.deliveryFeePence)}</dd>
        </div>
        <div>
          <dt className="inline text-charcoal-mid">Total: </dt>
      ...
      {quote.terms && (
        <p className="mt-2 whitespace-pre-line text-xs font-medium text-charcoal-mid">
          Terms: {quote.terms}
        </p>
      )}
      ...
      {stage === 'view' && canSelect && quote.status === 'submitted' && (
        <Button
          className="mt-3 w-full rounded-xl bg-brand py-3 font-bold text-white hover:bg-brand-dark"
          onClick={onChoose}
      ...
      {quote.status === 'accepted' && (
        <p className="mt-3 rounded-full bg-brand/10 px-3 py-1 text-center text-xs font-bold text-brand-dark">
          ✓ Selected
        </p>
      )}
