 ...
 *
 * ?occasion=<slug> pre-selects Step 1 so the "Full event" bundle card on
 * every occasion page lands the user on the right option automatically.
 */
...
function CateringFunnel() {
  ...
  const update = useCallback((partial: Partial<FunnelState>) => {
    ...
    // Clear field errors when the user changes input
  ...
  useEffect(() => {
    const handle = (e: KeyboardEvent) => {
      ...
      if (tag === 'TEXTAREA') return; // let enter create newlines in textarea
      if (tag === 'BUTTON') return; // don't double-fire button clicks
