...
describe('Part B admin action propagation (factory-backed API contracts)', () => {
  ...
  beforeAll(async () => {
    if (!configured) {
      throw new Error(
        `PART_B_E2E_CREDENTIALS_REQUIRED: ${[
  ...
  it('refunds through the admin API and exposes the customer status, vendor payout deduction, and balanced ledger pair', async () => {
    ...
    expect(ledger.map((entry) => entry.type)).toEqual(expect.arrayContaining(['refund', 'credit']));
  ...
  it('makes approval public, keeps rejection private with its reason, and propagates dispute outcomes to both contracts', async () => {
    const menu =
      (await factory.prisma.menu.findFirst({ where: { vendorId: moderationVendor.vendorId! } })) ??
