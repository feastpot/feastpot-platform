...
export function ShareClient({ canonicalLink, slug, businessName }: ShareClientProps) {
  ...
  // Generate client-side QR for instant preview. The canonical link is the
  // single source of truth - if the vendor's slug ever changes the server
  // re-renders this page and passes the new link, which then re-triggers
  // this effect and regenerates the preview automatically.
  useEffect(() => {
    let cancelled = false;
    async function gen() {
      try {
        ...
        const dataUrl = await QRCode.toDataURL(canonicalLink, {
      ...
      <section aria-labelledby="share-link-heading">
        ...
        <div className="rounded-xl border border-border bg-white p-3">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => copyToClipboard(canonicalLink, 'link')}
