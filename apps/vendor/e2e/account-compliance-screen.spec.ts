...
import {
  installComplianceMocks,
  makeEnforcementAction,
} from './helpers/compliance-mocks';
...
test(
  'A1: Standing section renders first and an active suspension card is visible without scrolling',
  async ({ page }) => {
    ...
    const standingHeading = page
      .getByRole('heading', { name: /standing/i })
      .first();
    ...
    const suspensionCard = page
      .getByText(/suspension|suspended/i)
      .first();
...
test(
  'A2: all seven PLATFORM_FACTS.vendorRequirements render verbatim in the Compliance section',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    await installComplianceMocks(page);
    ...
    let count = 0;
    for (const requirement of VENDOR_REQUIREMENTS) {
      const visible = await page.getByText(requirement).first().isVisible().catch(() => false);
      if (visible) count++;
    }
    expect(
      count,
...
test(
  'A3: logo and cover images upload and render without a full page reload',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    await expect(
      page.getByText(/uploading/i).or(page.locator('[aria-label="Uploading"]')).first(),
...
test(
  'A4: when an upload fails the slot shows a specific inline error and never a broken image icon',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    expect(
      ...
      'A4: error message must not be empty',
    ).toBeGreaterThan(0);
    ...
    expect(brokenCount, 'A4: no broken img element (empty or missing src) must appear after failed upload').toBe(0);
    ...
    console.log(`A4 PASS: upload failure shows "${errorText.trim()}", no broken icon, no orphaned file`);
  },
);
...
test(
  'A5: /compliance redirects to /account-and-compliance',
  async ({ page }) => {
    await installComplianceMocks(page);
...
test(
  'A5: /account-status redirects to /account-and-compliance',
  async ({ page }) => {
    await installComplianceMocks(page);
...
test(
  'A5: /terms redirects to /account-and-compliance',
  async ({ page }) => {
    await installComplianceMocks(page);
    ...
    expect(
      new URL(page.url()).pathname,
      'A5: /terms must redirect to /account-and-compliance',
    ).toBe('/account-and-compliance');
