 ...
 */
async function addChip(page: import('@playwright/test').Page, placeholder: string, value: string) {
...
async function expectChip(page: import('@playwright/test').Page, value: string) {
  await expect(page.locator('span').filter({ hasText: value }).first()).toBeVisible({
    timeout: 3_000,
  });
}
...
test('P1: complete an empty profile until the completeness list clears - under 4 min - zero navigations', async ({
  page,
}) => {
  ...
  await page
    ...
    .fill('Authentic West African home cooking from Peckham.');
  ...
  await expect(page.getByRole('button', { name: LIVE_ITEM_A.name }).locator('svg')).toBeVisible({
    timeout: 3_000,
  });
...
test('P2: add three cuisine chips - three separate stored values - not one comma-joined string', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
  ...
  const removeButtons = page.getByRole('button', {
    name: /^Remove (Nigerian|Caribbean|Ghanaian)$/,
  });
...
test('P3: featured dish removed automatically when the dish moves to draft in Menu', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
...
test('P4: @handle in Instagram field is stored as a valid full URL', async ({ page }) => {
  const m = new PageMetrics(page);
  await m.install();
  ...
  expect(socialLinks.instagram, 'P4: @mamanskitchen must be stored as the full Instagram URL').toBe(
    'https://www.instagram.com/mamanskitchen',
  );

  // Must not start with '@'.
  expect(socialLinks.instagram?.startsWith('@'), 'P4: stored value must not start with @').toBe(
    false,
  );
  ...
  console.log(
    'P4 complete: @mamanskitchen stored as https://www.instagram.com/mamanskitchen - PASS',
  );
});

// ── P5: Slug change - confirmation names QR consequence - old slug redirects ──

test('P5: slug change shows QR-code warning - old slug resolves via redirect', async ({ page }) => {
  const m = new PageMetrics(page);
  await m.install();
  ...
  const savePromise = captureNextProfileSave(page, makeVendorProfile({ slug: newSlug }));
  ...
  await expect(page.getByText(`feastpot.co.uk/vendors/${originalSlug}`)).toBeVisible();
  ...
  await expect(page.getByText(/QR code/i).or(page.getByText(/slug/i))).toBeVisible({
    timeout: 5_000,
  });

  const body = await savePromise;
  expect((body as { slug?: string }).slug, 'P5: new slug must be sent to the API').toBe(newSlug);
...
test('P6: typing in the business name field updates the live preview instantly without a save', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
...
test('P7: exceeding the 12-speciality limit blocks input with an explanation - nothing is silently truncated', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
  ...
  for (const s of SPECIALITIES) {
    expect(specialities as string[], `P7: "${s}" must be in the stored specialities`).toContain(s);
  }

  m.assertNoNavigation('P7');

  console.log(
    'P7 complete: 13th speciality blocked, exactly 12 stored, no silent truncation - PASS',
  );
});
