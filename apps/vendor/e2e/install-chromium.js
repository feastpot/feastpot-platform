...
function findCacheDir() {
  ...
  const candidates = [
    ...
    path.join(__dirname, '..', '..', '.cache', 'ms-playwright'), // apps/vendor -> workspace root
  ...
  // Fall back to what Playwright would compute even if the dir doesn't exist yet.
  return candidates[0];
}
  ...
  for (const platformDirName of fs.readdirSync(versionDir)) {
    ...
    for (const binaryName of fs.readdirSync(platformDir)) {
      ...
      if (stat.isSymbolicLink()) {
        ...
        // Back up the original so `npx playwright install` can restore it later.
