...
test(
  'VB1: suspended vendor sees the red non-dismissible banner on the dashboard',
  async ({ page }) => {
...
test(
  'VB2: suspended vendor with expired document gets a /compliance action link',
  async ({ page }) => {
...
test(
  'VB3: suspended vendor with no document issue gets an appeals mailto link',
  async ({ page }) => {
...
test(
  'VB4: renewal-due vendor sees the amber banner with a date (not a day count) and a dismiss button',
  async ({ page }) => {
...
test(
  'VB5: dismissed renewal banner stays hidden after a page reload within the same session',
  async ({ page }) => {
    await installVerificationBannerMocks(page, makeRenewalDueRecord());
    await page.goto('/');
...
test(
  'VB6: verified vendor sees no verification banner on the dashboard',
  async ({ page }) => {
...
test(
  'VB7: when overallState is SUSPENDED only the red banner shows, never the amber renewal banner',
  async ({ page }) => {
