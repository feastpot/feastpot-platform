...
test(
  'D1: set service area with radius - chips change when radius changes - under 60 s',
  async ({ page }) => {
    ...
    await expect(
      page.getByText('No districts selected.', { exact: false }),
    ).toBeVisible();
...
test(
  'D2: remove generated postcode - add manual postcode outside radius - both persist after reload',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    const remainingCodes = DISTRICTS_5MI.filter((c) => c !== 'SW2').concat(['SE15']).sort();
    const putBody = await captureNextDeliveryPut(page, makeDeliveryConfig({
      kitchenPostcode: KITCHEN_POSTCODE,
...
test(
  'D3: [CRITICAL] saved service area postcodes appear in customer search - excluded postcodes do not',
  async ({ page }) => {
    ...
    if (!vendorId) {
      test.skip(
        true,
        'D3 requires TEST_VENDOR_ID set to the test vendor UUID in the database.',
      );
      return;
    }
...
test(
  'D4: free delivery threshold below minimum order - inline error - save blocked',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    await installDeliveryMocks(
      page,
      { postcodes: DISTRICTS_5MI, kitchenPostcode: KITCHEN_POSTCODE },
    );
...
test(
  'D5: remove all postcodes - clear warning that vendor will not be discoverable',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
...
test(
  'D6: every control on the delivery page produces an observable, correct consequence',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    await expect(page.getByText(KITCHEN_DISTRICT)).toBeHidden({ timeout: 3_000 }).catch(() => {
    ...
    await postcodeInput.blur();
    await expect(page.getByText(KITCHEN_DISTRICT)).toBeVisible({ timeout: 8_000 }).catch(() => {
    ...
    await expect(page.getByText('Service area')).toBeHidden({ timeout: 2_000 }).catch(() => {
      noEffect.push('Local delivery checkbox (unchecking did not hide Service area section)');
    });
    // Re-check it.
    await page.getByText('Local delivery').click();
    await expect(page.getByText('Service area')).toBeVisible({ timeout: 2_000 }).catch(() => {
    ...
    await expect(page.getByText('Collection address')).toBeVisible({ timeout: 2_000 }).catch(() => {
      noEffect.push('Collection checkbox (checking did not reveal Collection address section)');
    });
    // Uncheck it.
    await page.getByText('Collection').click();
    await expect(page.getByText('Collection address')).toBeHidden({ timeout: 2_000 }).catch(() => {
    ...
    await page.getByRole('heading', { name: 'Order rules' }).click();
    await expect(page.getByText(/£3\.50/)).toBeVisible({ timeout: 2_000 }).catch(() => {
    ...
    await expect(page.getByText(/£15\.00/)).toBeVisible({ timeout: 2_000 }).catch(() => {
    ...
    await expect(page.getByText(/free delivery/i)).toBeVisible({ timeout: 2_000 }).catch(() => {
