...
test('PF1: rate schedule renders all three commission tiers and is never the empty state', async ({
  page,
}) => {
  ...
  const rateSection = page.getByText(/commission rate|rate schedule|what you pay/i).first();
  ...
  const emptyState = page.getByText(/no rate|rates unavailable|could not load/i).first();
...
test(`PF2: fee-change notice period on the performance screen is ${FEE_CHANGE_NOTICE_DAYS} days (PLATFORM_FACTS.feeChangeNoticeDays)`, async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
  ...
  const feeChangeNoticePattern = new RegExp(`${FEE_CHANGE_NOTICE_DAYS}[- ]day[s]?[- ]notice`, 'i');
...
test('PF3: no copy claims the full food subtotal stays with the vendor; no double-hyphen (--) on the screen', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
...
test('PF4: hourly distribution is labelled with the Europe/London timezone; weekly chart axes carry date labels', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
...
test('PF5: one revenue definition is used consistently across the performance screen and is stated explicitly', async ({
  page,
}) => {
  const m = new PageMetrics(page);
  await m.install();
