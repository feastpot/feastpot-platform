...
test(
  'AV1: set weekend-only opening days and slot window from scratch - correct payload sent to API',
  async ({ page }) => {
    ...
    const { openingDays, slotOpenHour, slotCloseHour } = await new Promise<{
      ...
      void page.route(/\/v1\/vendors\/me\/availability$/, (route) => {
        ...
        const snapshot = makeAvailabilitySnapshot({ openingDays: body.openingDays, slotOpenHour: body.slotOpenHour, slotCloseHour: body.slotCloseHour });
...
test(
  'AV2: [CRITICAL] daily order cap reaches the customer-facing availability API after being saved',
  async ({ page }) => {
    const apiUrl = process.env.TEST_API_URL;
    const vendorId = process.env.TEST_VENDOR_ID;

    if (!apiUrl) {
      test.skip(true, 'AV2 requires TEST_API_URL pointing at the running NestJS API. Set it to run this integration assertion.');
      return;
    }
    if (!vendorId) {
    ...
    await page.route(/\/v1\/vendors\/me\/capacity/, (route) => { void route.continue(); });
    await page.route(/\/v1\/vendors\/me\/blackouts/, (route) => { void route.continue(); });
    ...
    console.log(`AV2 complete: ${m.elapsedSec().toFixed(1)} s - maxOrdersPerDay=1 confirmed in public API - PASS`);
  },
);
...
test(
  'AV3: [CRITICAL] prep lead time reaches the public API and excludes near-future dates',
  async ({ page }) => {
    const apiUrl = process.env.TEST_API_URL;
    const vendorId = process.env.TEST_VENDOR_ID;
    ...
    const publicSnap = await page.evaluate(
      async ({ url, id }: { url: string; id: string }) => {
        const res = await fetch(`${url}/v1/vendors/${id}/availability`);
        if (!res.ok) return { ok: false, status: res.status, prepLeadHours: null, slotOpenHour: null };
        const data = (await res.json()) as { prepLeadHours?: number | null; slotOpenHour?: number };
        return { ok: true, status: res.status, prepLeadHours: data.prepLeadHours ?? null, slotOpenHour: data.slotOpenHour ?? 11 };
      },
      { url: apiUrl, id: vendorId },
    );
...
test(
  'AV4: clear all opening days - save shows "at least one day" error - vendor cannot receive orders',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    console.log('AV4 complete: no-days error shown correctly - vendor cannot silently go dark - PASS');
  },
);
...
test(
  'AV5: every control on the availability page produces an observable, correct consequence',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    ...
    await monPill.click();
    await expect(monPill).toHaveAttribute('aria-pressed', 'false').catch(() => {
      noEffect.push('Mon day pill (clicking did not deactivate the pill)');
    });
    await monPill.click();
    await expect(monPill).toHaveAttribute('aria-pressed', 'true').catch(() => {
    ...
    await expect(openInput).toHaveValue('9').catch(() => {
    ...
    await expect(closeInput).toHaveValue('21').catch(() => {
    ...
    await expect(leadInput).toHaveValue('6').catch(() => {
    ...
    await expect(capOrdersInput).toHaveValue('15').catch(() => {
    ...
    await expect(capTraysInput).toHaveValue('30').catch(() => {
    ...
    const sameDaySwitch = page.getByText('Allow same-day orders').locator('..').locator('[role="switch"]');
    ...
    await expect(lolInput).toHaveValue('72').catch(() => {
      noEffect.push('Large-order lead time input (value did not change to 72)');
    });
    await expect(lotInput).toHaveValue('10').catch(() => {
    ...
    if (eventBefore === eventAfter) {
      noEffect.push('Event catering manual quote toggle (aria-checked did not change after clicking)');
    }
    ...
    if (await removeBlackoutBtn.isVisible()) {
      ...
      if (blackoutCountAfter >= blackoutCountBefore) {
        noEffect.push('Blackout date Remove button (row count did not decrease after clicking Remove)');
      }
    } else {
      noEffect.push('Blackout date Remove button (no existing blackout row to remove - fixture missing)');
    }
    ...
    await expect(
      page.getByRole('button', { name: /Remove capacity for/ }).first(),
    ).toBeVisible({ timeout: 3_000 }).catch(() => {
      noEffect.push('Capacity Set button (no capacity row appeared in the list after clicking Set)');
    });
