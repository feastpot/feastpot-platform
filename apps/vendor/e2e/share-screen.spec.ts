...
test(
  'S1 CRITICAL: share page exposes exactly one link, it uses the canonical referralUrl, and 0% commission is stated',
  async ({ page }) => {
    ...
    expect(urlCount, 'S1 CRITICAL: exactly one element must render the canonical referralUrl').toBeGreaterThanOrEqual(1);
    ...
    const srcVendorMatches = [
      ...pageContent.matchAll(/href="[^"]*[?&]src=vendor[^"]*"/g),
    ];
    ...
    await expect(
      page.getByText(new RegExp(`${VENDOR_REFERRED_PCT}%`)).first(),
    ...
    console.log('S1 CRITICAL PASS: one canonical link, no ?src=vendor divergence, 0% commission stated');
  },
);
...
test(
  'S2: exactly one share URL is visible on the page and it is the canonical referralUrl',
  async ({ page }) => {
    await installShareMocks(page);
    await page.goto('/share');
    await expect(page.getByText(CANONICAL_REFERRAL_URL)).toBeVisible({ timeout: 8_000 });
...
test(
  'S3: both PNG and SVG download buttons are present and point to the canonical referral URL',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    await installShareMocks(page);
...
test(
  'S4: clicking the share link records a click via the attribution API with the correct referralLinkId',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
...
test(
  'S5: the share page renders the canonical referralUrl correctly under an Instagram user agent',
  async ({ browser }) => {
...
test(
  'S6: every commission percentage on the share page matches PLATFORM_FACTS values',
  async ({ page }) => {
    const m = new PageMetrics(page);
    await m.install();
    await installShareMocks(page);
