...
**PROD_DATABASE_URL points to the wrong region (Aug 2026):** PROD_DATABASE_URL connects to `aws-1-eu-north-1.pooler.supabase.com` (Stockholm) while the actual production project is London (`aws-1-eu-west-2`). **Always use `PROD_DIRECT_URL` for any production psql or Prisma operation.** `prisma migrate resolve` with `DATABASE_URL=$PROD_DATABASE_URL` silently targets the wrong DB and reports P3012 "not in a failed state" even when the actual prod DB has a failed migration. Fallback: UPDATE `_prisma_migrations` directly via psql with `$PROD_DIRECT_URL`.
